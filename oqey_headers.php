...
*/
if (!empty($_SERVER['SCRIPT_FILENAME']) && 'oqey_headers.php' == basename($_SERVER['SCRIPT_FILENAME'])) die ('Please do not load this page directly. Thanks!');

  function oqey_header_init_method() { 
...
if(is_admin() && ($_GET['page']=='Manage_headers') ){
...
add_action('init', 'oqey_header_init_method');
...
<p>&nbsp;</p>
<p>&nbsp;</p>
<p><strong>Please Donate, help the plugin development.</strong></p><br />
...
$datele = $gal->id.'--'.$cookies.'--'.$_COOKIE[LOGGED_IN_COOKIE].'--'.wp_create_nonce('oqey-header-upload');
?>
...
jQuery.ajax({
  url: ajaxurl,
  type : "POST",
  data:  {action: 'GetAllHeaderImages'},
  cache: false,
  success: function(html){
    ...
    jQuery(".dblclick").editable( ajaxurl, { 
      ...
      cancel    : "Cancel",
 	  style  : "inherit",
 	  submitdata : function ( value, settings ) { return { "action": 'UpdateHeaderDescription' }; }
  });
...
jQuery.ajax({
  url: ajaxurl,
  type : "POST",
  data:  {action: 'DeleteHeaderImage', id: id},
  cache: false,
  success: function(html){
...
function saveOrder(){ //SaveHeaderImagesOrder
  var d = jQuery('#oqeysortable').sortable('serialize');	
  
  jQuery.ajax({
  url: ajaxurl,
  type : "POST",
  data:  {action: 'SaveHeaderImagesOrder', d: d},
  cache: false,
  success: function(html){
   jQuery("#message").html(html);
   setTimeout("clearDiv()",5000);
...
require("oqey_header_functions.php"); 
...
add_action( 'wp_ajax_GetAllHeaderImages', 'GetAllHeaderImages' );
function GetAllHeaderImages(){
    global $wpdb;
    $oqey_image_url = get_option('siteurl') . '/wp-content/oqey_headers_images/';
    $oqey_header = $wpdb->prefix . "oqey_header";
        
    $oqey_images = $wpdb->get_results(" SELECT * FROM $oqey_header WHERE oqey_h_exclude !=1 ORDER BY oqey_h_order ASC, oqey_h_id DESC");

if(!empty($oqey_images)){
...
$output .='<a href="#edit"  onclick="EditeazaD(\''.$oqey_image->oqey_h_id.'\'); return false;">
           <img src="'.oQeyHeaderPluginUrlS().'/images/settings.png" width="16" height="16" style="position:absolute; left: 402px; top: 130px;" title="Edit/Add description to this image" />
		   </a>';
...
//delete image  DeleteHeaderImage
add_action( 'wp_ajax_DeleteHeaderImage', 'DeleteHeaderImage' );
function DeleteHeaderImage(){
       
       global $wpdb;
       $oqey_header = $wpdb->prefix . "oqey_header";
 
		$get_title_id = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM $oqey_header WHERE oqey_h_id ='%d'  ", $_POST['id'] ) );
		
		$image_title = $get_title_id->oqey_h_link;	
	    
	    $delete_img = $wpdb->query( $wpdb->prepare( "DELETE FROM $oqey_header WHERE oqey_h_id = '%d' ", $_POST['id'] ) );		
...
add_action( 'wp_ajax_SaveHeaderImagesOrder', 'SaveHeaderImagesOrder' );
function SaveHeaderImagesOrder(){
global $wpdb;
    $oqey_header = $wpdb->prefix . "oqey_header";
    
    $imgs = explode("&", $_POST['d']);

foreach ($imgs as $position => $item){
    
   $item = str_replace("img_header_id[]=", "", $item);
...
add_action( 'wp_ajax_UpdateHeaderDescription', 'UpdateHeaderDescription' );
function UpdateHeaderDescription(){
    global $wpdb;
    $oqey_header = $wpdb->prefix . "oqey_header";

$id = str_replace("d", "", $_POST['id']);
